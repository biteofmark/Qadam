...
export default function TestPage() {
  ...
  // Read navigation state to support review mode (from ResultsPage)
  useEffect(() => {
    try {
      const navState = window.history.state;
      if (navState && navState.state && navState.state.review) {
        setIsReviewMode(true);
        if (navState.state.userAnswers) {
          setUserAnswers(navState.state.userAnswers);
        }
      }
    } catch (e) {
      // ignore
              ...
              <CardContent className="space-y-6">
                ...
                <div className="space-y-3">
                  {currentQuestion?.answers.map((answer, index) => {
                    const selected = userAnswers[currentQuestion.id] === answer.id;
                    const correct = (answer as any).isCorrect === true;
                    const showCorrect = isReviewMode && correct;
                    const showIncorrect = isReviewMode && selected && !correct;
                    ...
                    return (
                      <div key={answer.id} className="flex items-start space-x-3">
                        <input
                          ...
                          value={answer.id}
                          checked={selected}
                          onChange={() => !isReviewMode && handleAnswerSelect(currentQuestion.id, answer.id)}
                          className="mt-1"
                        ...
                        <label
                          ...
                          className={`flex-1 p-3 rounded-lg border border-border transition-colors ${showCorrect ? 'bg-green-100 border-green-300' : ''} ${showIncorrect ? 'bg-red-100 border-red-300' : ''}`}
                        >
